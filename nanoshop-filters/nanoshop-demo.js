    ...
    $("#spotlight-button").click(function () {
        ...
        renderingContext.putImageData(
            Nanoshop.applyFilter(
                ...
                // This is a basic "darkener."
                function (x, y, r, g, b, a) {
                    var distanceFromCenter = function( x, y ) {
                        var xSquared = Math.pow( canvas.width / 2 - x, 2 );
                        var ySquared = Math.pow( canvas.height / 2 - y, 2 );
                        return Math.sqrt( xSquared + ySquared );
                    };
                    var usDistance = distanceFromCenter( x, y );
                    var maxDistance = distanceFromCenter( 0, 0 );
                    var ratio = (maxDistance - usDistance) / maxDistance;

                    var brightness = 2.5 * ratio;

                    var brightenPixel = function( val ){
                        return Math.min(255, (1 + val) * brightness );
                    };
