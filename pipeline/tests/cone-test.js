/*
 * Unit tests for our cone object.
 */
$(function () {
    ...
    test("Cone", function () {
        let res = 4;
        let shape = new Cone(GLSLUtilities, gl, true, { r: 0.0, g: 0.0, b: 0.5 }, res);
        equal(shape, shape, "Should be equal to self");
        notEqual(shape.duplicate(), shape, "Should not be equal to a duplicate");

        deepEqual(shape.vertices.length, 36, "Vertices length (res 4)");

        shape = new Cone(GLSLUtilities, gl, false, { r: 0.0, g: 0.0, b: 0.5 }, res);
        deepEqual(shape.vertices.length, 72, "Vertices length (not filled, res 4)");

        res = 20;
        shape = new Cone(GLSLUtilities, gl, true, { r: 0.0, g: 0.0, b: 0.5 }, res);
        deepEqual(shape.vertices.length, 180, "Vertices length (res 20)");

        shape = new Cone(GLSLUtilities, gl, false, { r: 0.0, g: 0.0, b: 0.5 }, res);
        deepEqual(shape.vertices.length, 360, "Vertices length (not filled, res 20)");

        res = 40;
        shape = new Cone(GLSLUtilities, gl, true, { r: 0.0, g: 0.0, b: 0.5 }, res);
        deepEqual(shape.vertices.length, 360, "Vertices length (res 40)");

        shape = new Cone(GLSLUtilities, gl, false, { r: 0.0, g: 0.0, b: 0.5 }, res);
        deepEqual(shape.vertices.length, 720, "Vertices length (not filled, res 40)");
